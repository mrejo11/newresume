    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col items-center text-center gap-8 max-w-4xl mx-auto">
          {/* Main Heading */}
          <h1 className={`text-4xl md:text-6xl lg:text-7xl font-extrabold leading-tight transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <div className={`flex flex-col sm:flex-row gap-4 justify-center transition-all duration-1000 delay-300 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
            ...
            <button
              ...
              onClick={() => window.dispatchEvent(new CustomEvent("generatePDF"))}
            >
          ...
          {/* Scroll Indicator */}
          <div 
            className={`flex flex-col items-center gap-3 absolute left-1/2 -translate-x-1/2 bottom-10 transition-all duration-1000 delay-500 cursor-pointer group ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`} 
            aria-hidden="true"
            onClick={scrollToContent}
          >
            <span className="text-sm font-medium text-slate-600 group-hover:text-slate-800 transition-colors">اسکرول کنید</span>
